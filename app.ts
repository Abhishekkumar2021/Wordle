...
const newGame = document.querySelector(`.new`) as HTMLDivElement;
const alertBox = document.querySelector(".alert") as HTMLDivElement;
const randomWord = ()=>{
    return words[Math.floor(Math.random()*words.length)]
}

function includes(word: string, words: string[]): boolean{
...
document.addEventListener("keydown", (e) => {
    if(e.key==='Backspace'){
        const button = document.querySelector(`#clear`) as HTMLButtonElement;
        button.click();
    }
    else if(e.key==='Enter'){
        const button = document.querySelector(`#enter`) as HTMLButtonElement;
        button.click();
    }
